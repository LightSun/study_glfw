...
typedef struct {
    ...
    void set(std::string  _data,std::string  _language,hb_script_t _scrpt,hb_direction_t _direction){
        data = _data;
        language = _language;
        script = _scrpt;
...
class HBShaper {
    public:
        HBShaper();
        HBShaper(const char* fontName, FreeTypeLib* lib);
        ~HBShaper();
        void set(const char* fontName, FreeTypeLib* lib);
